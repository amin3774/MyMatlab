...
%like |[a,b]|.
clc;clear;close all
PrismCode = fileread("Test5.sm"); % The PRISM code file main.sm is
% the prism model created in prism.
%%
out1=system('prism Test5.sm -mdp -noprobchecks -exporttrans prism_with_action.txt');%main.sm must be in the current folder of Matlab
if out1  %report an error if prism file is not generated.
    error('"prism_with_action.txt" is not generated.')
end
[out1,~]=system('prism Test5.sm -exporttransdotstates prism_tuple.txt');
if out1  %report an error if prism file is not generated.
