...
%like |[a,b]|.
clc;clear;close all
PrismCode = fileread("MasterSt.sm"); % The PRISM code file main.sm is
% the prism model created in prism.
%%
out1=system('prism MasterSt.sm -mdp -noprobchecks -exporttrans prism_with_action.txt');%main.sm must be in the current folder of Matlab
if out1  %report an error if prism file is not generated.
    error('"prism_with_action.txt" is not generated.')
end
[out1,~]=system('prism MasterSt.sm -exporttransdotstates prism_tuple.txt');
if out1  %report an error if prism file is not generated.
...
%         % sol = solve(prob,x0,'Options',options);
        ...
        for j=1:length(var7) %Outside the current scope.
            ...
            %% Modify the "system".
%             kol=[0,cell2mat(syncs(var7(j)-number_of_modules,5))'+1, length(main_system)];
%             system_modified= [main_system(kol(2)+1 : kol(3))];
%             system_modified=[main_system(1 : kol(2)),char(A.action(ref_t)), ',',system_modified ];
%             
%             comb=part1 + newline;
%             for k=1:number_of_modules
%                 comb=comb + newline + Single_modul(k);
%             end
%             comb=comb + newline + 'system' + newline + system_modified + newline + 'endsystem';
%             fid = fopen('mod.sm','wt');
%             fprintf(fid, comb);
%             fclose(fid);
%             % New Neighborhoods for the modified system.
%             Script_New_must_cannot_may;
            
            %%%%%%%
            % var2 is a set of the sync sets inside the scope.
            im=cell2mat(alls.child(var7(j),[3,4]));%imidiate children of var7(j)
            ...
            children=children(find (children));
            for k=1:length(children)
                if ~any(Rate.(char(modules(children(k)))).(char(A.action(ref_t)))(:))% if there are any "action"-transitions in a seq. process, then add that process to RIS(t)
            ...
            %% Modified system
            kol=[0,cell2mat(syncs(add_action-number_of_modules,5))'+1, length(main_system)];
            for k=1:length(add_action)+1
                tikke{k}= [main_system(kol(k)+1 : kol(k+1))];
                if k~=length(add_action)+1
