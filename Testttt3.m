...
while ~isempty(Tmod)
    ...
    for j=1:length(var2)
        ...
        if flag % no type A sp. tr., then check for type B.
            kol=[0,cell2mat(syncs(var2(j)-number_of_modules,5))'+1, length(main_system)];
            for k=1:2
                tikke{k}= [main_system(kol(k)+1 : kol(k+1))];
                if k~=length(var2(k))+1
                    tikke{1,k}=[tikke{1,k},char(A.action(ref_t)), ',' ];
                end
            end
            ...
            Px=cell2mat(alls.child(var2(j),3:end));
            Px=Px(Px<=number_of_modules);
            Px=nonzeros(Px);%line 74 of algo: Set of all seq. components below var2(j)
            for k=1:length(indice2)
                if flag2==false
                    break
                end
                if rem(indice2(k)) % skip transitions in other scopes.
                    continue
                end
                PS_cur=cell2mat(A.PS(indice2(k)));%original PS
                Pi_set=setdiff(Px,PS_cur); %line74 of algo: Pi = Px \ PS(t)
                
                Script_New_must_cannot_may;
                te2=[];te_must=[];
                cannots=[];
                for h=1:length(PS_cur)
                    cannots=[cannots,New_act.(A.action{ref_t}).(char(modules(PS_cur(h)))).CANNOT];
                end
                cannots=unique(cannots);
                for h=1:length(PS_cur)
                    %te2 is the union of new must and may neighborhoods.
                    te_must=[te_must;New_act.(A.action{ref_t}).(char(modules(PS_cur(h)))).MUST];
                    te2=[te2;   union(New_act.(A.action{ref_t}).(char(modules(PS_cur(h)))).MUST,...
                        setdiff(New_act.(A.action{ref_t}).(char(modules(PS_cur(h)))).MAY,cannots))];
                end
                te2=unique(te2);te_must=unique(te_must);
                for h=1:length(Pi_set)
                    if flag2==false
                        break
                    end
                    
                    if ismember(Pi_set(h),te2)%te2 is the union of new must and may neighborhoods, excluding cannots.
                        %%%%
                        node=Pi_set(h);
                        nodes_with_action=cell2mat(actions(find(ismember( actions(:,1) , A.action{ref_t})),2:end));
                        while node~=var2(j)
                            if ismember(node, nodes_with_action)
                              break
                            end                              
                            [node,~]=find(cell2mat(alls.child(number_of_modules+1:end,3:4))==node);
                            node=node+number_of_modules;% "node" is the smallest subtree with action sync and contain Pi_set(h)
                        end
                        node_ch=nonzeros(cell2mat(alls.child(node,3:end)));%children of "node"
                        %ind=find(node_ch==Pi_set(h));
                        % In "node" we should check for type B sp. tr.
                        state_with_added_selfloops=A.matrix(indice2(k),Pi_set(h));
                        if Rate.(char(modules(Pi_set(h)))).(A.action{ref_t})...
                                (state_with_added_selfloops,state_with_added_selfloops)
                            continue %If there already exists a selfloop in this state, skip the iteration.
                        end
%                         reachables=unique((Tuple(:,node_ch+1)),'rows');
%                         ind2=find(reachables(:,ind)==state_with_added_selfloops);
%                         any(Rate.modules(reachables()))
                        
%                         find(Tuple(:,Pi_set(h)+1)==state_with_added_selfloops)
                        [~,number1]=regexp(Single_modul_PRIME(Pi_set(h)),'\n','match');
                        state_name=Single_modul_PRIME(Pi_set(h));
                        ...
                        module_with_added_selfloops=char(Single_modul_PRIME(Pi_set(h)));
                        self=join(['[',A.action(ref_t),'] ',state_name,'=',string(state_with_added_selfloops)...
                            ,' -> 1 : (',state_name,'''=',string(state_with_added_selfloops),');'],'');
                        New_Rate.(char(modules(Pi_set(h)))).(char(A.action(ref_t)))(state_with_added_selfloops,state_with_added_selfloops)=1;
                        self=strjoin (self,'\n');
                        ...
                        nochange=setdiff(node_ch,Pi_set(h));
                        uni=union(nochange, Pi_set(h));
                        uni=sort(uni);
                        new_model=part1;
                        for q=1:length(uni)
                            if uni(q)==Pi_set(h)
                                new_model=new_model+newline;
                                new_model=strcat(new_model,char(module_with_added_selfloops));
                            else
                                new_model=new_model+newline;
                                new_model=strcat(new_model,Single_modul_PRIME(uni(q)));
                            end
                        end
...
%                         new_model=string(strjoin(new_model,'\n'));
                        new_model=new_model + newline + 'system' + newline + syncs{node-number_of_modules,7} + newline + 'endsystem';
                        fid = fopen('mod.sm','wt');
                        ...
                        fid = fopen('mod.txt', 'r');
                        out=fgetl(fid);
                        Trans_new = textscan(fid,'%f %*f %f %*f %*s');
                        Trans_new= cell2mat(Trans_new);
                        fclose(fid);
                        new=cell2mat(textscan(out,'%n %*n %n'));
                        old_model=strjoin(Single_modul_PRIME(node_ch),'\n');
                        old_model=strcat(part1,old_model);
                        old_model=string(strjoin(old_model,'\n'));
                        old_model=old_model + newline + 'system' + newline + syncs{node-number_of_modules,7} + newline + 'endsystem';
                        fid = fopen('mod.sm','wt');
                        fprintf(fid, old_model);
                        fclose(fid);
                        system('prism mod.sm -mdp -noprobchecks -exporttrans mod.txt');
                        fid = fopen('mod.txt', 'r');
                        out=fgetl(fid);
                        Trans_old = textscan(fid,'%f %*f %f %*f %*s');
                        Trans_old= cell2mat(Trans_old);
                        fclose(fid);
                        old=cell2mat(textscan(out,'%n %*n %n'));
                        if isequal(new,old) && isequal(Trans_new,Trans_old)%i.e. if the numbers of transitions and states do not change with added selfloop(s).
                            % if this condition is true, it means that
                            % the old and new systems have the same
                            % number of states and trs. Also the
                            % transitions in each system are identical.
                            % So, we are allowed to add selfloop to
                            % this state of this seq. component.
                            Single_modul_PRIME(Pi_set(h))=module_with_added_selfloops;

                        elseif ismember(Pi_set(h),te_must)
                            flag2=false;% it breaks this for loop and the also the previous for and continues from the next node of var2 in line 1201
                            break
                        end
...
%     if ~isempty(add_action) % if there are some sync sets inside the current scope that action can be added safely.
...
%             if rem(indice2(j)) % skip transitions in other scopes.
%                 continue
%             end
%             Pi_set=setdiff(alls.child(),cell2mat(A.PS(indice2(j)))); %line68 of algo: Pi \in IS(t)\PS(t)
%             nochange=setdiff(1:number_of_modules,Pi_set);
%             new_model=strjoin(Single_modul(nochange),'\n');
%             new_model=strcat(part1,new_model);
%             te1=cell2mat(A.PS(indice2(j)));%original PS
%             Script_New_must_cannot_may;
%             te2=[];
%             for k=1:length(te1)
%                 %temp2 is the union of new must and may neighborhoods.
%                 te2=[te2;union(New_act.(A.action{ref_t}).(char(modules(te1(k)))).MUST,New_act.(A.action{ref_t}).(char(modules(te1(k)))).MAY)];
%             end
%             te2=unique(te2);
%             for k=1:length(Pi_set)
%                 if ismember(Pi_set(k),te2)
%                     %%%%
%                     state_with_added_selfloops=A.matrix(indice2(j),Pi_set(k));
%                     [~,number1]=regexp(Single_modul(Pi_set(k)),'\n','match');
%                     state_name=Single_modul(Pi_set(k));
%                     state_name=char(state_name);
%                     state_name=state_name(number1(1)+1:number1(2)-1);
%                     [~,number2]=regexp(state_name,':','match');
%                     state_name=state_name(1:number2(1)-1);
%                     state_name= state_name(find(~isspace(state_name)));
%                     module_with_added_selfloops=char(Single_modul(Pi_set(k)));
%                     self=join(['[',A.action(ref_t),'] ',state_name,'=',string(state_with_added_selfloops)...
%                         ,' -> 1 : (',state_name,'''=',string(state_with_added_selfloops),');'],'');
%                     New_Rate.(char(modules(Pi_set(k)))).(char(A.action(ref_t)))(state_with_added_selfloops,state_with_added_selfloops)=1;
%                     self=strjoin (self,'\n');
%                     module_with_added_selfloops=strjoin([module_with_added_selfloops(1:number1(2)+1),self,module_with_added_selfloops(number1(2)+1:end)],'\n');
%                     new_model={char(new_model),char(module_with_added_selfloops)};
...
%         
        ...
        for j=1:length(RIS_ref)
            state_with_added_selfloops=unique(A.matrix(indice2,RIS_ref(j)));
            ...
            for k=1:length(state_with_added_selfloops)
                self(k)=join(['[',A.action(ref_t),'] ',state_name,'=',string(state_with_added_selfloops(k))...
                    ,' -> 1 : (',state_name,'''=',string(state_with_added_selfloops(k)),');'],'');
                New_Rate.(char(modules(RIS_ref(j)))).(char(A.action(ref_t)))(state_with_added_selfloops(k),state_with_added_selfloops(k))=1;
            end
            self=strjoin (self,'\n');
            module_with_added_selfloops=strjoin([module_with_added_selfloops(1:number1(2)+1),self,module_with_added_selfloops(number1(2)+1:end)],'\n');
            new_model={char(new_model),char(module_with_added_selfloops)};
    ...
    for j=1:length(var7) %Outside the current scope.
        ...
        for k=1:length(RIS_ref)
            state_with_added_selfloops=unique(A.matrix(indice2,RIS_ref(k)));
            ...
            for h=1:length(state_with_added_selfloops)
                self(h)=join(['[',A.action(ref_t),'] ',state_name,'=',string(state_with_added_selfloops(h))...
                    ,' -> 1 : (',state_name,'''=',string(state_with_added_selfloops(h)),');'],'');
                New_Rate.(char(modules(RIS_ref(k)))).(char(A.action(ref_t)))(state_with_added_selfloops(h),state_with_added_selfloops(h))=1;
            end
            self=strjoin (self,'\n');
            module_with_added_selfloops=strjoin([module_with_added_selfloops(1:number1(2)+1),self,module_with_added_selfloops(number1(2)+1:end)],'\n');
            new_model={char(new_model),char(module_with_added_selfloops)};
