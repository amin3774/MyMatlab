function y=gurtest2(in)
%Amin: If the system of equations has no solution, we can use this function
%to find the modification factors F'. It finds the modification factors
%-----> which are originally one<----------.
...
m.A = sparse(ones(1,num_var_x+num_var_y+num_var_f));
m.A(1)=1;
m.sense = '<';
m.rhs = 100;
...
m.obj(num_var_x+num_var_y+1:end)=-2;
m.Q=sparse(diag([zeros(1,num_var_x+num_var_y),ones(1,num_var_f)]));
